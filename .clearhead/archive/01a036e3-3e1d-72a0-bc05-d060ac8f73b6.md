...
alias: vevent-module
parent: 019f5841-1012-7fa2-9d9d-57dec5d906c7
state: Closed
---
...
A calendar resource should instead represent a **Plan**: the scheduling relationship attached to an Action. The configured Plan codec may be VEVENT or VTODO, with VEVENT as the default because it is the broadly interoperable calendar representation. Both codecs represent the same Plan domain relationship, while their integration profiles intentionally expose different peer-editable fields.
...
The Plan-native cutover is complete. One-off and materialized recurring
realizations reconcile through durable Plan/occurrence identity and field-wise
merge bases. Recurring moves update the same canonical `RECURRENCE-ID` override
without changing Action identity. VTODO occurrence task fields retain the full
profile, including terminal peer edits flowing through lineage snapshot and
single-token advancement. `TZID`-qualified recurrence keys canonicalize to the
same instant as UTC peers.

`sync calendar` now provides the compatibility/migration boundary. It adopts
UUID and projection-store-proven legacy VTODO identities without duplicating
Actions, previews codec changes, defers conversion while conflicts remain, and
converts each master plus same-UID overrides after field reconciliation in the
existing stale-guarded cross-mount transaction. VEVENT-to-VTODO migration seeds
the richer profile from the reconciled native Action; repeated synchronization
is byte-idempotent. The CLI help, man page, Radicale/vdirsyncer cookbook,
process contract, and normative iCalendar specification describe the final
behavior.
