...
- **2026-08-30 — Close-out verification passed.** CLI and LSP guidance now
  links to the process specification and limits itself to presentation and
  remediation behavior. The exact pinned platform composition passed every
  repository-owned gate, Core and CLI spec-conformance suites, independent
  rdflib execution of the governed `unscheduled` query, and schema validation.
  The stale external proof of retired `next-actions` was replaced with the
  authoritative `unscheduled` regression. Priority inheritance and context
  composition remain explicitly unresolved policy edges recorded below; they
  are deferred rather than being designed or encoded under this charter.
